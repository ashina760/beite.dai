...
class ExcelProcessor:
    ...
    def has_multiple_sheets(self):
        ...
        if len(self.workbook.sheetnames) > 1:
            return f"err: {self.workbook_name} is not only one sheet"
            ...
            if isinstance(cell, tuple):  # 防止用户传了 "A1:A5" 之类的区域
                for c in cell:
                    if c.value is None or str(c.value).strip() == "":
                        errors.append(f"❌ {c.coordinate} is empty")
            else:
                if cell.value is None or str(cell.value).strip() == "":
                    errors.append(f"{ref} is empty")
    ...
    def find_empty_cells(self,min_col=3, max_col=11):
        ...
        # for row in self.sheet.iter_rows(min_row=self.min_row, max_row=self.max_row, min_col=8, max_col=10):  # H~J
        #     for cell in row:
        #         if cell.value is None or str(cell.value).strip() == "":
        #             empty_cells.append(cell.coordinate)
        return empty_cells
